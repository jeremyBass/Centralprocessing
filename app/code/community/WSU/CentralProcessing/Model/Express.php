...
/**
 *
 * Centralprocessing Express Module
 */
class Wsu_Centralprocessing_Model_Express extends Mage_Payment_Model_Method_Abstract
    implements Mage_Payment_Model_Recurring_Profile_MethodInterface
{
    protected $_code  = Wsu_Centralprocessing_Model_Config::METHOD_WPP_EXPRESS;
	protected $_isInitializeNeeded      = true;
    protected $_formBlockType = 'centralprocessing/express_form';
     ...
     *
     * @var Wsu_Centralprocessing_Model_Pro
     */
    protected $_pro = null;
    protected $_proType = 'centralprocessing/pro';
    /**
    ...
    public function __construct($params = array())
    {
        ...
        if ($proInstance && ($proInstance instanceof Wsu_Centralprocessing_Model_Pro)) {
	...
	*/
     ...
     * @param float $amount
     * @return Wsu_Centralprocessing_Model_Express
     */
    public function order(Varien_Object $payment, $amount)
    {
        ...
        if ($payment->getIsTransactionPending()) {
            $message = Mage::helper('centralprocessing')->__('Ordering amount of %s is pending approval on gateway.', $formatedPrice);
            $state = Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW;
        } else {
            $message = Mage::helper('centralprocessing')->__('Ordered amount of %s.', $formatedPrice);
        }
        ...
        if ($payment->getIsTransactionPending()) {
            $message = Mage::helper('centralprocessing')->__('Authorizing amount of %s is pending approval on gateway.', $formatedPrice);
            $state = Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW;
            if ($payment->getIsFraudDetected()) {
                $status = Mage_Sales_Model_Order::STATUS_FRAUD;
            }
        } else {
            $message = Mage::helper('centralprocessing')->__('Authorized amount of %s.', $formatedPrice);
        }
        ...
        if ($payment->getAdditionalInformation($this->_isOrderPaymentActionKey)) {
            ...
            if ($authorizationTransaction->getIsClosed() || $voided) {
                if ($payment->getAdditionalInformation($this->_authorizationCountKey) > $maxAuthorizationNumber - 1) {
                    Mage::throwException(Mage::helper('centralprocessing')->__('The maximum number of child authorizations is reached.'));
                }
    ...
    public function acceptPayment(Mage_Payment_Model_Info $payment)
    {
        ...
        return $this->_pro->reviewPayment($payment, Wsu_Centralprocessing_Model_Pro::PAYMENT_REVIEW_ACCEPT);
    }
    ...
    public function denyPayment(Mage_Payment_Model_Info $payment)
    {
        ...
        return $this->_pro->reviewPayment($payment, Wsu_Centralprocessing_Model_Pro::PAYMENT_REVIEW_DENY);
    }
    ...
    public function getCheckoutRedirectUrl()
    {
        ...
        return Mage::getUrl('centralprocessing/express/start', array('_secure' => true));//
    }
    ...
    public function submitRecurringProfile(Mage_Payment_Model_Recurring_Profile $profile,
        ...
        $guid = $paymentInfo->
            getAdditionalInformation(Wsu_Centralprocessing_Model_Express_Checkout::PAYMENT_INFO_TRANSPORT_GUID);
    ...
    public function assignData($data)
    {
        ...
        $key = Wsu_Centralprocessing_Model_Express_Checkout::PAYMENT_INFO_TRANSPORT_BILLING_AGREEMENT;
        if (is_array($data)) {
    ...
    protected function _placeOrder(Mage_Sales_Model_Order_Payment $payment, $amount)
    {
        ...
        // prepare api call
        $guid = $payment->getAdditionalInformation(Wsu_Centralprocessing_Model_Express_Checkout::PAYMENT_INFO_TRANSPORT_GUID);
        $api = $this->_pro->getApi()
            ->setGuid($guid)
            ->setPayerId($payment->
                getAdditionalInformation(Wsu_Centralprocessing_Model_Express_Checkout::PAYMENT_INFO_TRANSPORT_PAYER_ID))
            ->setAmount($amount)
            ->setPaymentAction($this->_pro->getConfig()->paymentAction)
            ->setNotifyUrl(Mage::getUrl('centralprocessing/ipn/'))
            ->setInvNum($order->getIncrementId())
            ->setCurrencyCode($order->getBaseCurrencyCode())
            ->setCentralprocessingCart(Mage::getModel('centralprocessing/cart', array($order)))
     ...
     *
     * @param Wsu_Centralprocessing_Model_Api_Nvp
     * @param Mage_Sales_Model_Order_Payment
     */
    protected function _importToPayment($api, $payment)
    {
        $payment->setTransactionId($api->getTransactionId())->setIsTransactionClosed(0)
            ->setAdditionalInformation(Wsu_Centralprocessing_Model_Express_Checkout::PAYMENT_INFO_TRANSPORT_REDIRECT,
        ...
        if ($api->getBillingAgreementId()) {
            $payment->setBillingAgreementData(array(
                ...
                'method_code'           => Wsu_Centralprocessing_Model_Config::METHOD_BILLING_AGREEMENT
            ));
        }
     ...
     * @return Wsu_Centralprocessing_Model_Api_Abstract
     */
    ...
    protected function _isTransactionExpired(Mage_Sales_Model_Order_Payment_Transaction $transaction, $period)
    {
        ...
        /**
         * 11:49:00 Centralprocessing transactions closing time
         */
