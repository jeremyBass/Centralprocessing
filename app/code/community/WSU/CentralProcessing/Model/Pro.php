...
/**
 * Centralprocessing Website Payments Pro implementation for payment method instaces
 * This model was created because right now Centralprocessing Direct and Centralprocessing Express payment methods cannot have same abstract
 */
class Wsu_Centralprocessing_Model_Pro
{
    /**
     ...
     * Config instance
     *
     * @var Wsu_Centralprocessing_Model_Model_Config
     */
     ...
     * API instance
     *
     * @var Wsu_Centralprocessing_Model_Model_Api_Nvp
     */
    protected $_api = null;

    /**
     * Centralprocessing info object
     *
     * @var Wsu_Centralprocessing_Model_Model_Info
     */
     ...
     * @var string
     */
    protected $_apiType = 'centralprocessing/api_nvp';
     ...
     *
     * @param Wsu_Centralprocessing_Model_Model_Config $instace
     * @param int $storeId
     */
    public function setConfig(Wsu_Centralprocessing_Model_Model_Config $instace, $storeId = null)
    {
     ...
     *
     * @return Wsu_Centralprocessing_Model_Model_Config
     */
     ...
     *
     * @return Wsu_Centralprocessing_Model_Model_Api_Nvp
     */
     ...
     *
     * @return Wsu_Centralprocessing_Model_Model_Pro
     */
     ...
     *
     * @return Wsu_Centralprocessing_Model_Model_Info
     */
    public function getInfo()
    {
        if (null === $this->_infoInstance) {
            $this->_infoInstance = Mage::getModel('centralprocessing/info');
        }
     ...
     *
     * @param Wsu_Centralprocessing_Model_Model_Api_Abstract $from
     * @param Mage_Payment_Model_Info $to
     * @return Wsu_Centralprocessing_Model_Model_Pro
     */
    public function importPaymentInfo(Varien_Object $from, Mage_Payment_Model_Info $to)
    {
        // update Centralprocessing-specific payment information in the payment object
        $this->getInfo()->importToPayment($from, $to);

        /**
         * Detect payment review and/or frauds
         * Centralprocessing pro API returns fraud results only in the payment call response
         */
        if ($from->getDataUsingMethod(Wsu_Centralprocessing_Model_Model_Info::IS_FRAUD)) {
    ...
    public function void(Varien_Object $payment)
    {
        if ($authTransactionId = $this->_getParentTransactionId($payment)) {
            ...
            Mage::throwException(Mage::helper('centralprocessing')->__('Authorization transaction is required to void.'));
        }
    }
    ...
    public function refund(Varien_Object $payment, $amount)
    {
        ...
        if ($captureTxnId) {
            ...
            $api->setRefundType($isFullRefund ? Wsu_Centralprocessing_Model_Model_Config::REFUND_TYPE_FULL
                : Wsu_Centralprocessing_Model_Model_Config::REFUND_TYPE_PARTIAL
            );
            $api->callRefundTransaction();
            $this->_importRefundResultToPayment($api, $payment, $canRefundMore);
        } else {
            Mage::throwException(Mage::helper('centralprocessing')->__('Impossible to issue a refund transaction because the capture transaction does not exist.'));
        }
    }
    ...
    {
        return Wsu_Centralprocessing_Model_Model_Info::isPaymentReviewRequired($payment);
    }
    ...
    public function validateRecurringProfile(Mage_Payment_Model_Recurring_Profile $profile)
    {
        ...
        if (strlen($profile->getSubscriberName()) > 32) { // up to 32 single-byte chars
            $errors[] = Mage::helper('centralprocessing')->__('Subscriber name is too long.');
        }
        $refId = $profile->getInternalReferenceId(); // up to 127 single-byte alphanumeric
        if (strlen($refId) > 127) { //  || !preg_match('/^[a-z\d\s]+$/i', $refId)
            $errors[] = Mage::helper('centralprocessing')->__('Merchant reference ID format is not supported.');
        }
        ...
        if (strlen($refId) > 127) { //  || !preg_match('/^[a-z\d\s]+$/i', $scheduleDescr)
            $errors[] = Mage::helper('centralprocessing')->__('Schedule description is too long.');
        }
        if ($errors) {
     ...
     *
     * @param Wsu_Centralprocessing_Model_Model_Api_Nvp
