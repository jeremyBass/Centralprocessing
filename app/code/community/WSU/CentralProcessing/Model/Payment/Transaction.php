 ...
 *
 * @method Wsu_Centralprocessing_Model_Resource_Payment_Transaction _getResource()
 * @method Wsu_Centralprocessing_Model_Resource_Payment_Transaction getResource()
 * @method string getTxnId()
 * @method string getCreatedAt()
 * @method Wsu_Centralprocessing_Model_Payment_Transaction setCreatedAt(string $value)
 *
 * @category    Wsu
 * @package     Wsu_Centralprocessing
 * @author      jeremybass <jeremy.bass@wsu.edu>
 */
class Wsu_Centralprocessing_Model_Payment_Transaction extends Mage_Core_Model_Abstract
{
    /**
     ...
     * @var string
     */
    protected $_eventPrefix = 'centralprocessing_payment_transaction';
     ...
     * @var string
     */
    protected $_eventObject = 'centralprocessing_payment_transaction';
    ...
    protected function _construct()
    {
        ...
        $this->_init('centralprocessing/payment_transaction');
     ...
     * @param string $txnId
     * @return Wsu_Centralprocessing_Model_Payment_Transaction
     */
     ...
     * @param string $key
     * @param mixed $value
     * @return Wsu_Centralprocessing_Model_Order_Payment_Transaction
     * @throws Mage_Core_Exception
     */
    public function setAdditionalInformation($key, $value)
    {
        if (is_object($value)) {
            Mage::throwException(Mage::helper('centralprocessing')->__('Payment transactions disallow storing objects.'));
        }
    ...
    protected function _verifyTxnId($txnId)
    {
        if (null !== $txnId && 0 == strlen($txnId)) {
            Mage::throwException(Mage::helper('centralprocessing')->__('Transaction ID must not be empty.'));
        }
    }
    ...
    protected function _verifyThisTransactionExists()
    {
        ...
        if (!$this->getId()) {
            Mage::throwException(Mage::helper('centralprocessing')->__('This operation requires an existing transaction object.'));
        }
    }
}
