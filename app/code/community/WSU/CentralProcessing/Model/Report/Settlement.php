...
/*
 * Centralprocessing Settlement Report model
 *
 * Perform fetching reports from remote servers with following saving them to database
 * Prepare report rows for Wsu_Centralprocessing_Model_Report_Settlement_Row model
 *
 */
/**
 * Enter description here ...
 *
 * @method Wsu_Centralprocessing_Model_Resource_Report_Settlement _getResource()
 * @method Wsu_Centralprocessing_Model_Resource_Report_Settlement getResource()
 * @method string getReportDate()
 * @method Wsu_Centralprocessing_Model_Report_Settlement setReportDate(string $value)
 * @method string getAccountId()
 * @method Wsu_Centralprocessing_Model_Report_Settlement setAccountId(string $value)
 * @method string getFilename()
 * @method Wsu_Centralprocessing_Model_Report_Settlement setFilename(string $value)
 * @method string getLastModified()
 * @method Wsu_Centralprocessing_Model_Report_Settlement setLastModified(string $value)
 *
 * @category    Wsu
 * @package     Wsu_Centralprocessing
 * @author      jeremybass <jeremy.bass@wsu.edu>
 */
class Wsu_Centralprocessing_Model_Report_Settlement extends Mage_Core_Model_Abstract
{
    /**
     * Default Centralprocessing SFTP host
     * @var string
     */
    const REPORTS_HOSTNAME = "reports.centralprocessing.com";

    /**
     * Default Centralprocessing SFTP host for sandbox mode
     * @var string
     */
    const SANDBOX_REPORTS_HOSTNAME = "reports.sandbox.centralprocessing.com";

    /**
     * Centralprocessing SFTP path
     * @var string
     */
    ...
    protected $_csvColumns = array(
        'old' => array(
            'section_columns' => array(
                ...
                'TransactionID' => 1,
                'InvoiceID' => 2,
                'CentralprocessingReferenceID' => 3,
                'CentralprocessingReferenceIDType' => 4,
            ...
            'rowmap' => array(
                ...
                'CentralprocessingReferenceID' => 'centralprocessing_reference_id',
                'CentralprocessingReferenceIDType' => 'centralprocessing_reference_id_type',
        ...
        'new' => array(
            'section_columns' => array(
                ...
                'Transaction ID' => 1,
                'Invoice ID' => 2,
                'Centralprocessing Reference ID' => 3,
                'Centralprocessing Reference ID Type' => 4,
            ...
            'rowmap' => array(
                ...
                'Centralprocessing Reference ID' => 'centralprocessing_reference_id',
                'Centralprocessing Reference ID Type' => 'centralprocessing_reference_id_type',
    ...
    protected function _construct()
    {
        ...
        $this->_init('centralprocessing/report_settlement');
    }
    ...
    public function fetchAndSave($config)
    {
        ...
        foreach ($listing as $filename => $attributes) {
            $localCsv = tempnam(Mage::getConfig()->getOptions()->getTmpDir(), 'Centralprocessing_STL');
            if ($connection->read($filename, $localCsv)) {
                if (!is_writable($localCsv)) {
                    Mage::throwException(Mage::helper('centralprocessing')->__('Cannot create target file for reading reports.'));
                }
     ...
     * @return Wsu_Centralprocessing_Model_Report_Settlement
     */
    ...
    public function getFieldLabel($field)
    {
        switch ($field) {
            case 'report_date':
                return Mage::helper('centralprocessing')->__('Report Date');
            case 'account_id':
                return Mage::helper('centralprocessing')->__('Merchant Account');
            case 'transaction_id':
                return Mage::helper('centralprocessing')->__('Transaction ID');
            case 'invoice_id':
                return Mage::helper('centralprocessing')->__('Invoice ID');
            case 'centralprocessing_reference_id':
                return Mage::helper('centralprocessing')->__('Centralprocessing Reference ID');
            case 'centralprocessing_reference_id_type':
                return Mage::helper('centralprocessing')->__('Centralprocessing Reference ID Type');
            case 'transaction_event_code':
                return Mage::helper('centralprocessing')->__('Event Code');
            case 'transaction_event':
                return Mage::helper('centralprocessing')->__('Event');
            case 'transaction_initiation_date':
                return Mage::helper('centralprocessing')->__('Initiation Date');
            case 'transaction_completion_date':
                return Mage::helper('centralprocessing')->__('Completion Date');
            case 'transaction_debit_or_credit':
                return Mage::helper('centralprocessing')->__('Debit or Credit');
            case 'gross_transaction_amount':
                return Mage::helper('centralprocessing')->__('Gross Amount');
            case 'fee_debit_or_credit':
                return Mage::helper('centralprocessing')->__('Fee Debit or Credit');
            case 'fee_amount':
                return Mage::helper('centralprocessing')->__('Fee Amount');
            case 'custom_field':
                return Mage::helper('centralprocessing')->__('Custom');
            default:
                return $field;
        }
    }
    ...
    public function getSftpCredentials($automaticMode = false)
    {
        ...
        foreach(Mage::app()->getStores() as $store) {
            ...
            $active = (bool)$store->getConfig('centralprocessing/fetch_reports/active');
            if (!$active && $automaticMode) {
                continue;
            }
            $cfg = array(
                'hostname'  => $store->getConfig('centralprocessing/fetch_reports/ftp_ip'),
                'path'      => $store->getConfig('centralprocessing/fetch_reports/ftp_path'),
                'username'  => $store->getConfig('centralprocessing/fetch_reports/ftp_login'),
                'password'  => $store->getConfig('centralprocessing/fetch_reports/ftp_password'),
                'sandbox'   => $store->getConfig('centralprocessing/fetch_reports/ftp_sandbox'),
            );
