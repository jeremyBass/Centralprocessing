...
/**
 * Config model that is aware of all Wsu_Centralprocessing payment methods
 * Works with Centralprocessing-specific system configuration
 */
class Wsu_Centralprocessing_Model_Config
{
    /**
     * Centralprocessing Standard
     * @var string
     */
    const METHOD_WPS         = 'centralprocessing_standard';

    /**
     * Centralprocessing Website Payments Pro - Express Checkout
     * @var string
     */
    const METHOD_WPP_EXPRESS = 'centralprocessing_express';
    /**
     * Centralprocessing Website Payments Pro - Direct Payments
     * @var string
     */
    const METHOD_WPP_DIRECT  = 'centralprocessing_direct';
    /**
     * Direct Payments (Payflow Edition)
     * @var string
     */
    const METHOD_WPP_PE_DIRECT  = 'centralprocessinguk_direct';

    /**
     * Express Checkout (Payflow Edition)
     * @var string
     */
    const METHOD_WPP_PE_EXPRESS  = 'centralprocessinguk_express';    /**
     * Payflow Pro Gateway
     * @var string
     */
    ...
    const METHOD_BILLING_AGREEMENT  = 'centralprocessing_billing_agreement';
    ...
    /**
     * Default URL for centinel API (Centralprocessing Direct)
     *
     * @var string
     */
    public $centinelDefaultApiUrl = 'https://centralprocessing.wsu.edu';
    ...
    protected $_buildNotationPPMap = array(
        'centralprocessing_standard'  => 'WPS',
        'centralprocessing_express'   => 'EC'
    );
    ...
    protected $_ecStyleConfigMap = array(
        ...
        'centralprocessing_hdrimg' => 'hdrimg',
        'centralprocessing_hdrbordercolor' => 'hdrbordercolor',
        'centralprocessing_hdrbackcolor'   => 'hdrbackcolor',
        'centralprocessing_payflowcolor'   => 'payflowcolor',
    );

    /**
     * Currency codes supported by Centralprocessing methods
     *
     * @var array
     */
    ...
    /**
     * Merchant country supported by Centralprocessing
     *
     * @var array
     */
    ...
    /**
     * Buyer country supported by Centralprocessing
     *
     * @var array
     */
     ...
     *
     * @var array
     * @link https://cms.centralprocessing.com/us/cgi-bin/?cmd=_render-content&content_ID=developer/e_howto_api_ECButtonIntegration#id089QD0O0TX4__id08AH904I0YK
     */
    ...
    /**
     * Return merchant country codes supported by Centralprocessing
     *
     * @return array
     */
    ...
    /**
     * Return buyer country codes supported by Centralprocessing
     *
     * @return array
     */
     ...
     * @return string
     */
    public function getExpressCheckoutStartUrl($guid)
    {
        return $this->getCentralprocessingUrl(array(
    ...
    /**
     * Get url that allows to edit checkout details on centralprocessing side
     *
     * @param $guid
     * @return string
     */
    public function getExpressCheckoutEditUrl($guid)
    {
        return $this->getCentralprocessingUrl(array(
            'cmd'        => '_express-checkout',
            'useraction' => 'continue',
    ...
    /**
     * Get url for additional actions that Centralprocessing may require customer to do after placing the order.
     * For instance, redirecting customer to bank for payment confirmation.
     *
     * @param string $guid
     * @return string
     */
    public function getExpressCheckoutCompleteUrl($guid)
    {
        return $this->getCentralprocessingUrl(array(
     ...
     * @return string
     */
    public function getStartBillingAgreementUrl($guid)
    {
        return $this->getCentralprocessingUrl(array(
     ...
     /**
     * Centralprocessing web URL generic getter
     *
     * @param array $params
     * @return string
     */
    public function getCentralprocessingUrl(array $params = array())
    {
        return sprintf('https://%sewebservice.wsu.edu/CentralPaymentSite_WS/service.asmx%s',
            $this->sandboxFlag ? 'test-.' : '',
     ...
     * Centralprocessing will ignore "pal", if there is no total amount specified
     *
     * @param string $localeCode
     * @param float $orderTotal
     * @param string $pal encrypted summary about merchant
     * @see Centralprocessing_Model_Api_Nvp::callGetPalDetails()
     */
    public function getExpressCheckoutShortcutImageUrl($localeCode, $orderTotal = null, $pal = null)
    {
        ...
        return sprintf('https://www.centralprocessing.com/%s/i/btn/btn_xpressCheckout.gif',
            $this->_getSupportedLocaleCode($localeCode));
    }

    /**
     * Get Centralprocessing "mark" image URL
    ...
    public function getPaymentMarkImageUrl($localeCode, $orderTotal = null, $pal = null, $staticSize = null)
    {
        ...
        return sprintf('https://www.centralprocessing.com/%s/i/logo/Centralprocessing_mark_%s.gif',
            $this->_getSupportedLocaleCode($localeCode), $staticSize);
    }

    /**
     * Get "What Is Centralprocessing" localized URL
     * Supposed to be used with "mark" as popup window
     *
     * @param Mage_Core_Model_Locale $locale
     */
    public function getPaymentMarkWhatIsCentralprocessingUrl(Mage_Core_Model_Locale $locale = null)
    {
        $countryCode = 'US';
        if (null !== $locale) {
        ...
        return sprintf('https://www.centralprocessing.com/%s/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIsCentralprocessing-outside',
    ...
    {
        return sprintf('https://www.centralprocessing.com/%s/i/bnr/%s_solution_PP%s.gif',
            $this->_getSupportedLocaleCode($localeCode),
    ...
    public function getPaymentFormLogoUrl($localeCode)
    {
        ...
        switch ($locale) {
            ...
            case 'fr_FR':
                ...
                $imageType = 'bnr';
                $locale = 'en_US';
                $domain = 'centralprocessingobjects.com';
                break;
            case 'it_IT':
                ...
                break;
            default:
                $imageName='Centralprocessing_mark_60x38';
                $countryPrefix = '';
                break;
        }
    ...
    /**
     * Return supported types for Centralprocessing logo
     *
     * @return array
     */
    public function getAdditionalOptionsLogoTypes()
    {
        ...
        return array(
            'wePrefer_150x60'       => Mage::helper('centralprocessing')->__('We prefer Centralprocessing (150 X 60)'),
            'wePrefer_150x40'       => Mage::helper('centralprocessing')->__('We prefer Centralprocessing (150 X 40)'),
            'nowAccepting_150x60'   => Mage::helper('centralprocessing')->__('Now accepting Centralprocessing (150 X 60)'),
            'nowAccepting_150x40'   => Mage::helper('centralprocessing')->__('Now accepting Centralprocessing (150 X 40)'),
            'paymentsBy_150x60'     => Mage::helper('centralprocessing')->__('Payments by Centralprocessing (150 X 60)'),
            'paymentsBy_150x40'     => Mage::helper('centralprocessing')->__('Payments by Centralprocessing (150 X 40)'),
            'shopNowUsing_150x60'   => Mage::helper('centralprocessing')->__('Shop now using (150 X 60)'),
            'shopNowUsing_150x40'   => Mage::helper('centralprocessing')->__('Shop now using (150 X 40)'),
        );
    }

    /**
     * Return Centralprocessing logo URL with additional options
     *
    ...
    public function getAdditionalOptionsLogoUrl($localeCode, $type = false)
    {
        ...
        return sprintf('https://www.centralprocessingobjects.com/%s/i/bnr/bnr_%s.gif', $locale, $type);
    }
    ...
    public function getExpressCheckoutButtonFlavors()
    {
        ...
        return array(
            self::EC_FLAVOR_DYNAMIC => Mage::helper('centralprocessing')->__('Dynamic'),
            self::EC_FLAVOR_STATIC  => Mage::helper('centralprocessing')->__('Static'),
        );
    }
    ...
    public function getExpressCheckoutButtonTypes()
    {
        ...
        return array(
            self::EC_BUTTON_TYPE_SHORTCUT => Mage::helper('centralprocessing')->__('Shortcut'),
            self::EC_BUTTON_TYPE_MARK     => Mage::helper('centralprocessing')->__('Acceptance Mark Image'),
        );
    }
    ...
    public function getPaymentActions()
    {
        ...
        $paymentActions = array(
            self::PAYMENT_ACTION_AUTH => Mage::helper('centralprocessing')->__('Authorization'),
            self::PAYMENT_ACTION_SALE => Mage::helper('centralprocessing')->__('Sale')
        );
        if (!is_null($this->_methodCode) && $this->_methodCode == self::METHOD_WPP_EXPRESS) {
            $paymentActions[self::PAYMENT_ACTION_ORDER] = Mage::helper('centralprocessing')->__('Order');
        }
    ...
    public function getRequireBillingAddressOptions()
    {
        ...
        return array(
            self::REQUIRE_BILLING_ADDRESS_ALL       => Mage::helper('centralprocessing')->__('Yes'),
            self::REQUIRE_BILLING_ADDRESS_NO        => Mage::helper('centralprocessing')->__('No'),
            self::REQUIRE_BILLING_ADDRESS_VIRTUAL   => Mage::helper('centralprocessing')->__('For Virtual Quotes Only'),
        );
    }

    /**
     * Mapper from Centralprocessing-specific payment actions to Magento payment actions
     *
     * @return string|null
     */
     ...
     * "sole" = "Express Checkout for Auctions" - Centralprocessing allows guest checkout
     * "mark" = "Normal Express Checkout" - Centralprocessing requires to checkout with Centralprocessing buyer account only
     *
     * @return array
     */
    public function getExpressCheckoutSolutionTypes()
    {
        ...
        return array(
            self::EC_SOLUTION_TYPE_SOLE => Mage::helper('centralprocessing')->__('Yes'),
            self::EC_SOLUTION_TYPE_MARK => Mage::helper('centralprocessing')->__('No'),
        );
    }
    ...
    public function getExpressCheckoutBASignupOptions()
    {
        ...
        return array(
            self::EC_BA_SIGNUP_AUTO  => Mage::helper('centralprocessing')->__('Auto'),
            self::EC_BA_SIGNUP_ASK   => Mage::helper('centralprocessing')->__('Ask Customer'),
            self::EC_BA_SIGNUP_NEVER => Mage::helper('centralprocessing')->__('Never')
        );
    }
    ...
    /**
     * Payment data delivery methods getter for Centralprocessing Standard
     *
     * @return array
     */
...
//            self::WPS_TRANSPORT_IPN_PDT  => Mage::helper('adminhtml')->__('Both IPN and PDT'),
    ...
    /**
     * Return list of supported credit card types by Centralprocessing Direct gateway
     *
     * @return array
     */
    ...
    /**
     * Return list of supported credit card types by Centralprocessing Direct (Payflow Edition) gateway
     *
     * @return array
     */
    ...
    /**
     * Dynamic Centralprocessing image URL getter
    ...
    protected function _getDynamicImageUrl($type, $localeCode, $orderTotal, $pal)
    {
        ...
        return sprintf('https://fpdbs%s.centralprocessing.com/dynamicimageweb?%s',
    ...
    /**
     * Map Centralprocessing Standard config fields
     *
     * @param string $fieldName
     * @return string|null
     */
    ...
    /**
     * Map Centralprocessing Express config fields
     *
     * @param string $fieldName
     * @return string|null
     */
    ...
    /**
     * Map Centralprocessing Direct config fields
     *
     * @param string $fieldName
     * @return string|null
     */
    ...
    /**
     * Map Centralprocessing Website Payments Pro common config fields
     *
     * @param string $fieldName
     * @return string|null
     */
    protected function _mapWppFieldset($fieldName)
    {
        switch ($fieldName)
        {
            ...
            case 'proxy_port':
            case 'button_flavor':
                return "centralprocessing/wpp/{$fieldName}";
            default:
                return null;
        }
    }
    ...
    {
        $pathPrefix = 'centralprocessing/wpuk';
    ...
    /**
     * Map Centralprocessing common style config fields
     *
     * @param string $fieldName
     * @return string|null
     */
    protected function _mapGenericStyleFieldset($fieldName)
    {
        switch ($fieldName) {
            case 'logo':
            case 'page_style':
            case 'centralprocessing_hdrimg':
            case 'centralprocessing_hdrbackcolor':
            case 'centralprocessing_hdrbordercolor':
            case 'centralprocessing_payflowcolor':
                return "centralprocessing/style/{$fieldName}";
            default:
                return null;
        }
    }
    ...
    /**
     * Map Centralprocessing General Settings
     *
     * @param string $fieldName
     * @return string|null
     */
    protected function _mapGeneralFieldset($fieldName)
    {
        switch ($fieldName)
        {
            ...
            case 'merchant_country':
                return "centralprocessing/general/{$fieldName}";
            default:
                return null;
        }
    }
    ...
    public function getApiAuthenticationMethods()
    {
        ...
        return array(
            '0' => Mage::helper('centralprocessing')->__('API Signature'),
            '1' => Mage::helper('centralprocessing')->__('API Certificate')
        );
    }
    ...
    public function getApiCertificate()
    {
        ...
        return Mage::getModel('centralprocessing/cert')->loadByWebsite($websiteId, false)->getCertPath();
    }
}
